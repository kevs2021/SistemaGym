...
class Asistencias extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                $data[$i]['status'] = '<span class="btn btn-danger">En Gym</span>';
                $data[$i]['editar'] = '<button class="btn btn-primary" type="button" onclick="editarAsist(' . $data[$i]['id_asistencia'] . ');"><i class="fas fa-edit"></i></button>';
            } else {
                $data[$i]['editar'] = '';
