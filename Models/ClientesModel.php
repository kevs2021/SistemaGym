...
class ClientesModel extends Query{
    ...
    public function getClientes($estado = null)
    {
        // Si no se pasa estado, obtener todos los clientes
        if ($estado !== null) {
            $sql = "SELECT * FROM clientes WHERE estado = ?";
            return $this->selectAll($sql, [$estado]);
        } else {
            $sql = "SELECT * FROM clientes";  // Si no filtras, obten todos los clientes
    ...
    public function actualizarEstadoCliente($id_cliente, $estado)
    {
        $sql = "UPDATE clientes SET estado = ? WHERE id = ?";
        $datos = array($estado, $id_cliente);
        
        // Comprobamos si la actualización afectó alguna fila
        if ($this->save($sql, $datos)) {
            return true;
        }
        return false;
    }
